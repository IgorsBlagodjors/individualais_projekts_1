...
static void stringValidator(char *buffer, int maxLen, const string &prompt) {
  while (true) {
    ...
    if (cin.fail()) {
      ...
      cout << "\033[31mInput too long! Maximum " << (maxLen - 1)
           << " characters.\033[0m\n";
      continue;
    }
...
static int indexValidator(int size, const string &prompt) {
  ...
  while (true) {
    ...
    cout << "\033[31mInvalid selection. Choose 1-" << size
         << " or 0 to cancel: \033[0m";
  }
}

// Input validation for string (can leave the old, press Enter)
static void editStringValidator(char *buffer, int maxLen,
                                const string &fieldName) {
  while (true) {
    cout << "Current " << fieldName << ": " << buffer << endl;
    cout << "Press Enter to keep current value.\n";
    cout << "Enter new " << fieldName << " (max " << (maxLen - 1)
         << " chars): ";
  ...
  while (true) {
    ...
    try {
      ...
      if (value > 0 && value <= 9999) {
        ...
        cout << "\033[31mInvalid range. Enter a number from 1 to 9999 or press "
                "Enter to keep old value.\033[0m\n";
      }
    } catch (...) {
      cout << "\033[31mInvalid input. Enter a valid number or press Enter to "
  ...
  while (true) {
    ...
    try {
      ...
      if (value > 0.0 && value <= 9999.99) {
        ...
        cout << "\033[31mInvalid range. Enter a number from 0.01 to 9999.99 or "
                "press Enter to keep old value.\033[0m\n";
      }
    } catch (...) {
      cout << "\033[31mInvalid input. Enter a valid number or press Enter to "
              "keep old value.\033[0m\n";
...
static string dateValidCheck(const string &dateInput) {
  ...
  while (true) {
    ...
    int daysInMonth[] = {31, leap ? 29 : 28, 31, 30, 31, 30, 31, 31, 30, 31, 30,
                         31};
...
static void searchReceipts(Receipt *receipts, int count) {
  ...
  cout << "Enter product category (Food, Clothes, Electronics, Books, "
          "HomeAndGarden or leave empty): ";
  ...
  if (foundReceipts.empty()) {
    ...
    cout << "\n\033[32mFound " << foundReceipts.size()
         << " matching receipts:\033[0m\n";
    displayReceipts(foundReceipts.data(),
                    static_cast<int>(foundReceipts.size()));
  }
}
    ...
    if (loadedProducts[i].code == productCode) {
      if (quantity > loadedProducts[i].quantityInStock) {
        cout << "\033[31mNot enough stock for " << loadedProducts[i].name
             << "! Available: " << loadedProducts[i].quantityInStock
             << "\033[0m\n";
        return false;
      }
...
static PurchasedProduct inputNewProduct() {
  ...
  while (!(cin >> prodIndex) || prodIndex < 1 ||
         prodIndex > loadedProductCount) {
    cin.clear();
    cin.ignore(1000, '\n');
    cout << "\033[31mInvalid selection. Choose 1-" << loadedProductCount
         << ": \033[0m";
  }
  cin.ignore();
...
void saveProducts(const Product *arr, int size, const char *filename) {
  ...
  file << "]";
  cout << "\033[32mSaved " << size << " products to " << filename
       << "!\033[0m\n";
}
...
void saveEmployees(const Employee *arr, int size, const char *filename) {
  ...
  file << "]";
  cout << "\033[32mSaved " << size << " employees to " << filename
       << "!\033[0m\n";
}

// Save discount cards to JSON file
void saveDiscountCards(const DiscountCard *arr, int size,
                       const char *filename) {
  ofstream file(filename);
  file << "[\n";
  ...
  file << "]";
  cout << "\033[32mSaved " << size << " discount cards to " << filename
       << "!\033[0m\n";
}
...
void saveReceipts(const Receipt *arr, int size, const char *filename) {
  ofstream file(filename);
  if (!file.is_open()) {
    cerr << "\033[31mError: cannot open file " << filename
         << " for writing.\033[0m\n";
    return;
  }
  ...
  cout << "\033[32mSaved " << size << " receipts to " << filename
       << "!\033[0m\n";
}
...
void loadProducts(const char *filename, Product *&arr, int &size,
  ...
  while (getline(file, line)) {
    ...
    if (line.find("\"code\"") != string::npos) {
      ...
      sscanf_s(line.c_str(), "    \"quantityInStock\": %d,",
  ...
  file.close();
  cout << "\033[32mLoaded " << size << " products from " << filename
       << "\033[0m\n";
}
...
void loadEmployees(const char *filename, Employee *&arr, int &size,
  ...
  while (getline(file, line)) {
    ...
    if (line.find("\"id\"") != string::npos) {
      ...
      strncpy_s(current.firstName, sizeof(current.firstName), s.c_str(),
                _TRUNCATE);
    } else if (line.find("\"lastName\"") != string::npos) {
      size_t start = line.find('"', line.find(':')) + 1;
      size_t end = line.find('"', start);
      string s = line.substr(start, end - start);
      strncpy_s(current.lastName, sizeof(current.lastName), s.c_str(),
                _TRUNCATE);
  ...
  file.close();
  cout << "\033[32mLoaded " << size << " employees from " << filename
       << "\033[0m\n";
}
...
void loadDiscountCards(const char *filename, DiscountCard *&arr, int &size,
  ...
  while (getline(file, line)) {
    ...
    if (line.find("\"cardNumber\"") != string::npos) {
      ...
      size_t end = line.find('"', start);
      string s = line.substr(start, end - start);
      strcpy_s(current.ownerFirstName, sizeof(current.ownerFirstName),
               s.c_str());
  ...
  file.close();
  cout << "\033[32mLoaded " << size << " discount cards from " << filename
       << "\033[0m\n";
}
...
void loadReceipts(const char *filename, Receipt *&arr, int &size,
  ...
  while (getline(file, line)) {
    ...
    if (line.find("\"receiptNumber\"") != string::npos) {
      sscanf_s(line.c_str(), "    \"receiptNumber\": %d,",
  ...
  file.close();
  cout << "\033[32mLoaded " << size << " receipts from " << filename
       << "\033[0m\n";
}
...
void addProduct() {
  ...
  newProduct.code =
      intValidator("Enter product code (0 to cancel, max 4 digits): ", true);
  ...
  newProduct.quantityInStock =
      intValidator("Enter product quantity in stock (max 4 digits): ");

  int category;
  cout << "Select category (0-Food, 1-Clothes, 2-Electronics, 3-Books, "
          "4-HomeAndGarden): ";
  ...
  arrayExtension(loadedProducts, loadedProductCount, loadedProductCapacity,
                 newProduct);
...
void addEmployee() {
  ...
  newEmployee.id =
      intValidator("Enter employee ID (0 to cancel, max 4 digits): ", true);
  if (newEmployee.id == 0)
    return;

  stringValidator(newEmployee.firstName, 11,
                  "Enter first name (max 10 chars): ");
  stringValidator(newEmployee.lastName, 11, "Enter last name (max 10 chars): ");

  int dep;
  cout << "Select department (0-Food, 1-Clothes, 2-Electronics, 3-Books, "
          "4-HomeAndGarden): ";
  ...
  arrayExtension(loadedEmployees, loadedEmployeeCount, loadedEmployeeCapacity,
                 newEmployee);
...
void addDiscountCard() {
  ...
  card.cardNumber =
      intValidator("Enter card number (0 to cancel, max 4 digits): ", true);
  if (card.cardNumber == 0)
    return;

  stringValidator(card.ownerFirstName, 11,
                  "Enter owner's first name (max 10 chars): ");
...
void addReceipt() {
  ...
  receipt.receiptNumber =
      intValidator("Enter receipt number (0 to cancel, max 4 digits): ", true);
  ...
  while (true) {
    ...
    while (!(cin >> prodChoice) || (prodChoice != 0 && prodChoice != -1)) {
      cin.clear();
      cin.ignore(1000, '\n');
      cout << "\033[31mInvalid input. Enter 0 to finish or -1 to add new: "
              "\033[0m";
    }
    cin.ignore();
  ...
  while (!(cin >> cardIndex) || cardIndex < 1 || cardIndex > loadedCardCount) {
    cin.clear();
    cin.ignore(1000, '\n');
    cout << "\033[31mInvalid selection. Choose 1-" << loadedCardCount
         << ": \033[0m";
  }
  receipt.card = loadedCards[cardIndex - 1];

  // Save receipt
  arrayExtension(loadedReceipts, loadedReceiptCount, loadedReceiptCapacity,
                 receipt);
  ...
  while (true) {
    ...
    try {
      ...
      if (catVal >= 0 && catVal <= 4) {
        ...
        cout << "\033[31mInvalid category. Enter a number 0-4 or press Enter "
...
void editDiscountCard(DiscountCard *arr, int size) {
  ...
  int discountIndex =
      indexValidator(size, "Select discount card index (0 to cancel): ");
  if (discountIndex == 0) {
    cout << "\033[33mEdit cancelled.\033[0m\n";
    return;
  }
  ...
  while (true) {
    ...
    try {
      ...
      if (catVal >= 0 && catVal <= 2) {
        ...
        cout << "\033[31mInvalid type. Enter a number 0-2 or press Enter to "
                "keep old value.\033[0m\n";
      }
    } catch (...) {
  ...
  for (int i = 0; i < size; i++) {
    if (arr[i].receiptNumber == number) {
      ...
      while (true) {
        ...
        if (prodChoice > 0 &&
            prodChoice <= static_cast<int>(arr[i].items.size())) {
          ...
          int newIndex =
              indexValidator(loadedProductCount, "Enter new product index: ");
  ...
  do {
    ...
    case 4: {
      ...
      do {
        ...
        case 2:
          if (loadedEmployeeCount > 0)
            deleteItem(loadedEmployees, loadedEmployeeCount, displayEmployees,
                       "employee");
          else
            cout << "\033[1;31m\nNo employees available!\033[0m\n";
          break;
        ...
        case 4:
          if (loadedReceiptCount > 0)
            deleteItem(loadedReceipts, loadedReceiptCount, displayReceipts,
                       "receipt");
          else
            cout << "\033[1;31m\nNo receipts available!\033[0m\n";
          break;
    ...
    case 5:
      searchReceipts(loadedReceipts, loadedReceiptCount);
      break;
    case 6: {
      // Sort Data
      int sortChoice;
      do {
        ...
        switch (sortChoice) {
        case 1:
          sortProductsByName(loadedProducts, loadedProductCount);
          break;
        case 2:
          sortProductsByPriceAscending(loadedProducts, loadedProductCount);
          break;
        case 3:
          sortProductsByPriceDescending(loadedProducts, loadedProductCount);
          break;
        case 4:
          sortProductsByQuantityAscending(loadedProducts, loadedProductCount);
          break;
        case 5:
          sortProductsByQuantityDescending(loadedProducts, loadedProductCount);
          break;
        default:
          if (sortChoice != 6)
            cout << "\033[1;31m\nInvalid choice!\033[0m\n";
